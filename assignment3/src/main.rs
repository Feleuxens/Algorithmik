...
use crate::closure::{ClosureGraph, DGPQuery, separators_from_percentages};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // ---- Distance Closures (k-DGP) preprocessing ----------------------------
    // Separators are expressed as percentages of the maximum CH rank.
    // Fewer groups -> faster queries but denser closures (more memory); more
    // groups -> sparser closures but slower queries.

    let k = [70.0, 88.0, 96.0, 99.0];
    // let k = [70.0, 88.0, 96.0];
    // let k = [70.0, 88.0];
    // let k = [70.0];


    println!("Preprocessing distance closures");
    let start = Instant::now();
    let separators = separators_from_percentages(
        ch_graph.get_number_of_nodes(),
        &k,
    );
    let closure = ClosureGraph::build(&ch_graph, &separators);
    let closure_preprocess_time = start.elapsed();
    println!(
        "Closure preprocessing time: {}s ({} groups, {} closure edges)",
        closure_preprocess_time.as_secs(),
        closure.num_groups,
        closure.num_edges(),
    );
    ...
    let mut closure_query = DGPQuery::new(
        ch_graph.get_number_of_nodes() as usize,
        closure.num_groups as usize,
    );
    let mut results: Vec<(u32, u32, Option<u32>, u128)> = Vec::with_capacity(queries.len());
    for (source, target) in queries {
        let start = Instant::now();
        let result = closure_query.query(&closure, source, target);
        let elapsed = start.elapsed().as_micros();

        results.push((source, target, result, elapsed));
    }
    ...
    write_log_file("log.txt", file_read_time, preprocess_time, file_write_time, total_edges, num_edges, num_shortcuts, closure_preprocess_time, closure.num_groups, closure.num_edges())?;
...
pub fn write_log_file(path: &str, time_io_graph: Duration, time_ch_processing: Duration, time_io_ch_file: Duration, edge_count: u32, orig_edge_count: usize, shortcut: u32, closure_preprocess_time: Duration, closure_num_groups: u32, closure_num_edges: usize) -> Result<(), Box<dyn std::error::Error>> {
    let file = File::create(path)?;
    let mut writer = BufWriter::new(file);
    ...
    writeln!(writer, "Shortcut edge count: {}\n", shortcut)?;
    writeln!(writer, "Closure preprocess time: {}s, added {} closure in {} groups", closure_preprocess_time.as_secs(), closure_num_edges, closure_num_groups)?;
